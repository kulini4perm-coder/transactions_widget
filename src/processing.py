...
def sort_by_date(list_of_dicts: list[dict], value_date: str = "descending") -> list[dict]:
    """Функция сортирует список словарей по значению ключа 'date'"""

    if value_date == "descending":  # Направление сортировки: True для убывания, False для возрастания
        reverse_flag = True
    else:
        reverse_flag = False
