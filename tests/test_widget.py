...
def test_mask_account_card_for_card(input_card: str) -> None:  # Тест на определение карты
    assert mask_account_card(input_card) == "Visa Classic 1234 56** **** 5678"


def test_mask_account_card_for_account(input_account: str) -> None:  # Тест на определение счета
    assert mask_account_card(input_account) == "Счет **7890"


@pytest.mark.parametrize(
    "value, expected",
    [  # Параметризация для разных типов карт
...
def test_mask_account_card_type_card(value: str, expected: str) -> None:
    assert mask_account_card(value) == expected


@pytest.mark.parametrize(
    "value, expected",
    [  # Параметризация для разных типов счетов
        ("Счет 12345678901234567890", "Счет **7890"),
        ("Депозитный 00000000000000000000", "Депозитный **0000"),
        ("Валютный счет 99992222333344445555", "Валютный счет **5555"),
    ],
)
def test_mask_account_card_type_accounts(value: str, expected: str) -> None:
    assert mask_account_card(value) == expected


@pytest.mark.parametrize(
    "value, expected",
    [  # Параметризация в случае отсутствия названия карты/счета
...
def test_mask_account_card_name_missing(value: str, expected: str) -> None:
    assert mask_account_card(value) == expected
...
def test_get_date_different_dates(value: str, expected: str) -> None:
    assert get_date(value) == expected


def test_get_date_empty_string() -> None:  # С пустой входной строкой
