...
def test_get_mask_card_number_stand(number_card: str) -> None:  # Стандартный тест с фикстурами
    assert get_mask_card_number(number_card) == "1234 56** **** 3456"


@pytest.mark.parametrize(
    "value, expected",
    [  # Используем параметризацию для разнообразных номеров
...
def test_get_mask_card_number_diff_numb(value: str, expected: str) -> None:
    assert get_mask_card_number(value) == expected


def test_get_mask_card_number_extreme() -> None:  # Проверка исключений
...
def test_get_mask_account_stand(number_account: str) -> None:  # Стандартный тест с фикстурами
...
def test_get_mask_account_diff_numb(value: str, expected: str) -> None:
    assert get_mask_account(value) == expected


def test_get_mask_account_extreme() -> None:  # Проверка исключений
