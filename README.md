...
Transactions Widget «Виджет финансовых операций» - это программа для фильтрации и анализа банковских транзакций.
Поддерживает работу с данными в форматах JSON, CSV и Excel. 
Позволяет фильтровать операции по статусу, валюте, наличию ключевых слов в описании, а также сортировать их по дате.

## Ключевые функции

- Загрузка данных: Чтение транзакций из разных форматов с использованием pandas.
- Фильтрация: По статусу (EXECUTED, CANCELED, PENDING), по валюте (только рублевые операции) и по поисковому запросу в описании (с поддержкой регулярных выражений re).
- Аналитика: Подсчет количества операций по категориям.
- Безопасность: Маскирование номеров карт и счетов (формат XXXX XX** **** XXXX и **XXXX).
- Интерфейс: Интерактивное консольное меню для управления процессом обработки.
...
В текущей версии добавлена запись сообщений о работе функций модулей masks и utils.
Это позволит анализировать производительность приложения и усовершенствовать в следующих версиях.


## Чтение csv-, excel-файлов:

В текущей версии добавлена возможность чтения транзакций из csv-, excel-файлов.
Также проведена работа по отладке и тестированию новых функций.
